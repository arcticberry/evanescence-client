...
function ServiceItem({ vendors, service, selectedVendors, onVendorToggle, onServiceToggle }) {
			...
			<div className="card-header" id="headingFour">
				<div className="m-0">
					<div className="custom-control custom-checkbox">
						...
						<i className={`mdi mdi-chevron-${opened ? 'right' : 'down'} accordion-arrow`}></i>
					</div>
				</div>
			</div>
							...
							return (
								<div className="custom-control custom-checkbox" key={index}>
									...
									<label className="custom-control-label" htmlFor={vendorKey}>
										<span className="mr-2">
											<img style={{ width: '8em' }} alt="" src={vendor.brand_url} />
										</span>
									</label>
								</div>
							);
