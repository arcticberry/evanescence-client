...
import { applicationSchema, applicationsSchema } from "./application.schema";
...
export const toggleApplicationStatus = (application_id, payload) => async (
  dispatch
) => {
  try {
...
export const controlAppServiceActivation = (
  application_id,
  service_id,
  payload
) => async (dispatch) => {
  try {
    const response = await Axios.patch(
      `/service/activate/${application_id}/${service_id}`,
      payload
    );
    dispatch(toggleApplicationsFetching());
  } catch (error) {
